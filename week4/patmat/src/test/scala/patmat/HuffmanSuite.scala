...
class HuffmanSuite extends FunSuite {
  ...
  test("times on a simple test list") {
    assert(times(List('a','a','a')) === List(('a',3)))
  }

  test("times on a more complex test list") {
    assert(times(List('a','b','a','b','b')) === List(('a',2), ('b',3)) ||
           times(List('a','b','a','b','b')) === List(('b',3), ('a',2)))
  }
  ...
  test("combine on small tree / empty tree lists") {
  ...
  test("decode and quick encode work on a very short text") {
    new TestTrees {
